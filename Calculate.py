...
for p in player_list:
    if p.get_proj_score()/p.get_price() > 0.005 and p.get_name() not in \
            ["Bradley Beal", "Montrezl Harrell", "Willie Cauley-Stein", "TJ Warren", "Cody Zeller"]:
        new_player_list.append(p)

best = optimal_lineup(new_player_list,
                      [None, "Bradley Beal", "TJ Warren", "Montrezl Harrell", "Willie Cauley-Stein", None, None, "Cody Zeller"])
for p in best:
    if isinstance(p, str):
        print(p)
    else:
